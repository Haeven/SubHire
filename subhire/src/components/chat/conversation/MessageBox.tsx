...
import { useAppDispatch, useAppSelector, useGetUser } from "@/lib/hooks";
import { Message, User } from "@/lib/interfaces";
import { useFormatDate } from "@/lib/hooks";
import React, { memo, useEffect, useState } from "react";
import { ProfilePicture, SharedLayout, AppTooltip } from "@/components";
...
const MessageBox = ({
  ...
  useEffect(() => {
    if (isGroup) {
      getUserInfo(currentMsg.senderId).then((senderData: any) => {
        setSenderData(senderData);
      });
    }

    const date = formatDate((new Date(currentMsg.date)));
          ...
          >
            ...
            {isEditingThisMsg && (
              ...
              >
                ...
                < AppTooltip tip="cancel" position="top" />
              </button>
            )}
